...
class DashboardController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $totalAdmissionFees = AdmissionModel::where('status', 1)
            ...
            ->first()
            ->total_sum;
        $totalAdmissionPendingFees = AdmissionModel::where('status', 0)
            ->select(DB::raw('SUM(amount) as total_sum'))
            ->first()
            ->total_sum;
        $totalExpense = ExpenseModel::select(DB::raw('SUM(amount) as total_sum'))->first()->total_sum;
        $classList = ClassModel::all();
        $couseList = CourseModel::all();
        $teacherList = TeacherModel::all();


        return view('pages.admin.dashboard.dashboard', compact('totalStudent', 'totalTeacher', 'totalCourse', 'totalFees', 'classList', 'couseList', 'teacherList', 'pendingFees', 'totalAdmissionFees', 'totalAdmissionPendingFees','totalExpense'));
    }
}
