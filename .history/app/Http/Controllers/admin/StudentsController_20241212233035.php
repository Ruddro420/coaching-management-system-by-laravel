...
class StudentsController extends Controller
{
    ...
    public function getAdmissionData($studentId, $classname)
    {
        ...
        Log::debug('Searching for admission data with studentId: ' . $studentId . ' and classname: ' . $classname);
    
        // Query the AdmissionModel for the given studentId and classname
        $admissionData = AdmissionModel::where('student_id', $studentId)
                                       ->where('classname', $classname)
                                       ->first();
    
        if (!$admissionData) {
            return response()->json(['error' => 'Admission data not found'], 404);
        }
    
        // Return the admission data as a response
        return response()->json([
